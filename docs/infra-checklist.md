...
- [x] **[Sanh]** GitHub repo created at https://github.com/SanhVo2023/Matviet-ATS-platform — main branch + feat/01-foundation pushed
- [x] **[Claude]** Migrations 0001-0013 applied to live Supabase. RLS on all 20 tables. Seeds in. Advisor at 3 accepted-WARN findings (0 ERRORs).
- [x] **[Claude]** TypeScript types generated to `app/src/types/db.ts` from live schema.
- [x] **[Claude]** Brand derivatives generated (favicon, apple-touch-icon, og-image) via `npm run brand:generate` Sharp script.
- [x] **[Claude]** Auth + role-scoped layout shell live at `feat/01-foundation`. Login/reset-password pages, middleware session refresh, dashboard layout, role-aware sidebar, admin invite page.
...
- [x] `git init` in project root + first commit (Word docs included; tilde lock files excluded via `.gitignore`) — done 2026-04-28, commit `c67c767`
- [x] Create GitHub repo (chosen name: `Matviet-ATS-platform`); push initial commit — at https://github.com/SanhVo2023/Matviet-ATS-platform
- [ ] Install Node 20 LTS, npm/pnpm, Supabase CLI, Netlify CLI, Fly CLI, **GitHub CLI** (gh) — `gh` is missing; needed for future PR creation. Sanh: `winget install --id GitHub.cli` (Windows)
- [ ] Create Netlify account; connect GitHub repo; create site (suggested ID: `matviet-hr`)
...
- [x] **favicon (`src/app/icon.png`, 64×64)** — generated by `npm run brand:generate` (Sharp script)
- [x] **apple-touch-icon (`src/app/apple-icon.png`, 180×180)** — generated, navy bg + glyph
- [x] **og-image (`src/app/opengraph-image.png`, 1200×630)** — generated, brand-navy bg + MV6 lockup
...
- [ ] (v2) Vietnamese tagline overlay on og-image — needs font registration in Sharp pipeline

### Supabase Pro plan upgrade — [Sanh ↔ Claude] — DEFERRED to Group 11 (pre-launch)
...
- Note: Free tier is fine for build groups 1-10; Pro becomes valuable only when production traffic + PITR matter.
