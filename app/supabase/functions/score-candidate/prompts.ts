// Mirror of app/src/lib/ai/gemini/prompts.ts (Deno-compatible).

import type { CriterionCode, ParsedCv, Weights } from "./types.ts";
...
export function buildScoreSystemPrompt(extraNote = ""): string {
...
overall_summary: 2-3 câu tiếng Việt, đánh giá tổng thể cho HR (không phải cho ứng viên).${extraNote ? "\n\n" + extraNote : ""}`;
}
...
function stripRawText(parsed: ParsedCv): Omit<ParsedCv, "_raw_text"> {
  const copy = { ...parsed } as Partial<ParsedCv>;
