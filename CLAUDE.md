...
| `candidates` | Candidate CRUD, CV upload (đơn + bulk 1-20 PDF, AI backfill ô trống), stage transitions, `dossier.ts` — buildCandidateDossier: MD hồ sơ lắp TƯƠI (view, không store — ADR 0017). UI trang ứng viên = CÁI THANG (ADR 0019): `components/features/candidates/journey/*` + rail SlideOver; ngôn ngữ giai đoạn chung ở `lib/stage-visuals.ts` + `lib/next-actions.ts` | — |
