...
> **⚡ PLATFORM (post-pivot, ADRs 0009–0013):** The app runs on **Cloudflare (Workers + D1 + R2 + Queues + Cron + Durable Objects)** at `hr.matviet.com.vn`. The old Supabase+Netlify+Fly stack is fully removed from the repo (2026-07-16). DB work happens in `app/src/db/` (Drizzle) + `app/migrations-d1/`. The old master plan (`~/.claude/plans/mutable-crunching-coral.md` v5.0) is **superseded** where it conflicts with the ADRs. The app is also positioned as the base of an all-in employee management system (ADR 0012). **The HRM is now BUILT (v1 basics, H0–H3, 2026-09-08, LOCAL)** — see `docs/PRD-HRM.md` (v1.0) + ADR 0023 + the HRM Modules table below. Full agentic HRM (ATS = the Recruiting module), propose-first, traditional HRM UI: employees + org (H0), onboarding + contracts + compliance-clock agent (H1), leave (H2), offboarding + HR analytics (H3) — the whole hire→onboard→manage→leave lifecycle on one `person_id`. Migrations 0009–0014 are LOCAL only; **prod migrate + deploy pending** (all additive; 0014 = open-proposal partial unique index + one-time twin cleanup). Deferred: **D-FW** Fastwork integration (no API access), **D-PERF** performance/OKR (pending BOD), **D-ESS** employee self-service accounts (employee-auth decision), **H3b** announcements/documents. Since ADR 0020 the app is **agent-driven, propose-first**: agents prepare actions as feed cards; humans one-tap approve.
...
- **Post-audit RENOVATION COMPLETE (2026-09-07, all in LOCAL main)** — plan `how-about-the-ui-silly-origami`, trains R0–R5:
  - **R0 safety+access:** login redirect-loop fix, deactivation now writes better-auth `banned` (was `is_active`-only → never blocked sign-in), cookieCache removed (instant revoke), admin set-password, reset-email failure surfaced, per-email lockout (`login_attempts` + D1 rate-limit storage), safe-next open-redirect fix, `/khong-co-quyen`, demo lockdown. Brand unified in `src/lib/brand.ts` (email/PDF/Excel/QR). Playwright+axe infra.
  - **R1 stage collapse (ADR 0021):** 16→8 stages; `src/lib/stages.ts` single enum + `ALLOWED_TRANSITIONS`; `deriveCandidateStatus` (waiting-on + days-waiting); `transitionStage()` is THE guarded writer (reject-reason required, stage_history everywhere, approval-chain auto-cancel); `offer_accepted` made real. Migrations 0007+0008.
  - **R2 exec+mobile:** ApprovalDigestCard + read-only exec candidate view (scoped) + approval emails; md breakpoint; mobile drawer bell/account; role-aware BottomTabs.
  - **R3 ops:** action-inbox gaps, scoring reliability (dedupe/kill-switch guard/ownership guard/manual badge/bulk retry), owed-eval tab, `/nhat-ky`, invite email, `/huong-dan` SOP.
...
| `candidates` | Candidate CRUD, CV upload (đơn + bulk 1-20 PDF, AI backfill ô trống), stage transitions, `dossier.ts` — buildCandidateDossier: MD hồ sơ lắp TƯƠI (view, không store — ADR 0017). **Stages: 8 (ADR 0021), enum ở `lib/stages.ts` (SINGLE source + `ALLOWED_TRANSITIONS` guard). `transitionStage()` (service.ts) là writer DUY NHẤT: guard + reject-reason + `stage_history` mọi nơi + tự hủy chuỗi duyệt khi rời `approving`. `deriveCandidateStatus()` (`lib/candidate-status.ts`) tính waiting-on + days-waiting từ related rows → thay `stageReadiness`; `attachDerivedStatus()` (repository) batch cho kanban/table.** UI trang ứng viên = CÁI THANG (ADR 0019): `components/features/candidates/journey/*` + rail SlideOver; `lib/stage-visuals.ts` + `lib/next-actions.ts`; `RejectReasonDialog` bắt buộc lý do từ chối | — |
| `scoring` | AI scoring pipeline (CV→MD cache `cv_markdowns` cache-first qua `getOrCreateCvMarkdown` — đường đọc CV duy nhất cho AI (ADR 0017); extract-text.ts cũng powers upload prefill): `enqueueScoring` + `triggerScoring` (SCORING_QUEUE) → in-process `worker.ts` (Workers AI 2-pass via `toMarkdown`→parse→score, decoupled weights, evidence validation + anti-bluff cap 45; on success cũng seed `candidates.ai_summary` từ pass-2 overall_summary — ADR 0018) → cron drain `/api/scoring/drain` backstop. Manual-slider fallback for failed scoring | yes |
| `interviews` | Schedule/cancel + evaluations (Graph calendar/Teams); `ai-questions.ts` — generateAndPersistInterviewQuestions: câu hỏi PV sinh TỰ ĐỘNG khi đặt lịch (waitUntil, ADR 0018), lưu `interviews.ai_questions`, nút "Tạo lại" | — |
...
| `offers` | Offer magic link: `composeFromTemplate('offer')` mints 7-day token, injects `{{offer_link}}`; public `/nhan-viec/[token]` accept→`offer_accepted` (HR xác nhận → `hired`; ADR 0021 — không còn nhảy thẳng `hired`) / decline→`rejected`+`offer_declined` reason. `offer_response` recorded (offer-declines ≠ ordinary rejects). Group 13 onboarding trigger | yes |
| `reports` | 6 charts + PDF/Excel export + demo seeder; all queries take a `ReportFilter` | yes |
| `agent-flows` | Agent-driven hiring, propose-first (ADR 0020): emitters → `events.ts` → deterministic `generators.ts` (+ AI `job-from-intent.ts`) → `agent_proposals` → "Hôm nay" feed on dashboard → `execute.ts` chạy ĐÚNG các service thao tác tay dùng (audit `via:'agent_proposal'`). Per-job `HiringAgent` DO (`src/agents/hiring-agent.ts`, Agents SDK, binding `HIRING_AGENT`) giữ timer stale → alarm → `SELF` binding → `/api/agent/sweep`. ⚠️ Next bundle KHÔNG import gói `agents` (webpack vs `cloudflare:*`) — nói chuyện với DO qua `agent-link.ts` raw stub fetch. Test hooks: `/api/agent/ping`, `AGENT_STALE_OVERRIDE_SECONDS`. **Reconcile train (2026-09-08, agentic audit P1):** `reconcile.ts` = nightly truth-based backstop (new kinds `confirm_hire` / `retry_scoring` / `orphan_approval` + re-runs `sweepCandidate` for watched stages + supersedes cards on archived candidates) run by `daily.ts` → `/api/agent/daily` (cron pokes every minute after 01:00 UTC; runs AT MOST once per VN day behind the `agent_last_daily_sweep` settings marker, also runs the HRM compliance sweep; `?force=1` re-runs). `execute.ts` `preflightError` refuses stale cards (candidate moved stage). Dedupe race closed by partial unique index `uq_proposals_open_dedupe` (migration 0014). `proposalStats` + `AgentHealthCard` on `/cai-dat/he-thong`. **`server/ai/availability.ts` `aiAvailability()` is THE AI gate** (kill switch + cost breaker) — runtime + scoring claim guard both use it | yes |

### HRM modules (v1 basics, ADR 0023 + `docs/PRD-HRM.md`) — H0–H3 built 2026-09-08 (local)

| Module | What it does | README |
|---|---|---|
| `employees` (H0) | Employee records on the ADR 0012 person-centric core. `repository` (list joined person/dept/pos + resolved managers, detail, `employeeHeadcountStats` HR analytics) + `service` (create/update, `ensureEmployeeForCandidate` — **idempotent candidate→employee on the same `person_id`, auto on hire** via `ung-vien/actions changeStageAction`, find-or-creates position from job title). Routes `/nhan-vien` (list + [id] detail + inline edit). `nextEmployeeCode` tested | yes |
| `org` (H0) | Departments (tree) + positions. `listDepartmentsWithMeta`/`listPositions` (tallies), CRUD with guarded deletes. Route `/phong-ban` | yes |
| `contracts` (H1) | Labor contracts (Bộ luật LĐ 2019: thử việc/xác định/không xác định). `listActiveContractsEndingBefore` feeds the compliance sweep; `seedProbationContract`. ContractsCard on profile | yes |
| `onboarding` (H1) | New-hire checklist (`ONBOARDING_TEMPLATE`, idempotent seed). OnboardingChecklist on profile | yes |
...
**Deferred (need Sanh / external unlock):** Fastwork integration (**D-FW** — no API access yet), performance/OKR (**D-PERF** — pending BOD), employee self-service accounts (**D-ESS** — employee-auth decision; widens leave-submission + announcement/policy-ack reach). HRM migrations 0009–0014 applied LOCAL only — **prod migrate pending** (all additive; safe).

### UI system (UX batches 1–4, 2026-09-08) — read `app/.claude/CLAUDE.md` before UI work

Primitives are settled: `PageContainer` (wide/default/detail/narrow — every dashboard route), `PageHeader` (+`leading`), `EmptyState` (+`illustration`, 7 brand scenes in `components/brand/illustrations.tsx`), `StatusPill` (THE status language: success/warning/error/info/neutral/brand/accent), `ui/table` parts (THE list-table look), `DataTable` (sortable), `SimpleSelect`/`Combobox`/`Checkbox`/`DateInput` (form layer — no native selects/dates/checkboxes remain except the kanban mobile select), `PersonAvatar`, `StageGroupIcon`, `Skeleton*`, `MatVietGlyph` (brand mark SVG; PWA icons + manifest generated from it). AgentDock is a radix Dialog. Two-tap rule for people-affecting actions.
